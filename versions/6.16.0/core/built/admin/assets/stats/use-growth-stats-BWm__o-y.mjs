import { n as S, o as V, p as j, Z as o, h as F, g as G } from "./index-BxbeiQvY.mjs";
import { bn as K, bl as Q, bm as R } from "./stats-CtTAHMmO.mjs";
function Z(a) {
...
const q = (a, Y, h, d) => {
  ...
  const D = d || a[a.length - 1], i = a.length > 0 ? a[a.length - 1] : { free: 0, paid: 0, comped: 0 }, P = Y.length > 0 ? Y[Y.length - 1] : { mrr: 0 }, e = D.free + D.paid + D.comped, m = P.mrr, u = {
    total: "0%",
    free: "0%",
    paid: "0%",
    mrr: "0%"
  ...
  const h = [...a].sort((t, s) => new Date(t.date).getTime() - new Date(s.date).getTime()), d = [...Y].sort((t, s) => new Date(t.date).getTime() - new Date(s.date).getTime()), D = h.map((t) => t.date), i = d.map((t) => t.date), P = [.../* @__PURE__ */ new Set([...D, ...i])].sort((t, s) => new Date(t).getTime() - new Date(s).getTime());
  let e = null, m = null;
  ...
  var B;
  const { startDate: Y, endDate: h } = S(() => V(a), [a]), d = j(Y), D = a === 1 ? o(d).subtract(1, "day").format("YYYY-MM-DD") : d, { data: i, isLoading: P } = K({
    searchParams: {
      date_from: D
    }
  ...
  }, [t, s, d, (B = i == null ? void 0 : i.meta) == null ? void 0 : B.totals]), r = S(() => z(t, s), [t, s]), A = S(() => Z(g), [g]), L = S(() => P || m || y, [P, m, y]), _ = S(() => {
    if (!(u != null && u.stats))
      return [];
...
//# sourceMappingURL=use-growth-stats-BWm__o-y.mjs.map
